#include "dasta_6_packed_helib.h"

using namespace helib;

namespace DASTA_6_PACKED {
...
std::vector<Ctxt> DASTA_128_HElib::HE_decrypt(std::vector<uint8_t>& ciphertexts,
  ...
  for (size_t i = 0; i < num_block; i++) {
    ...
    for (unsigned r = 1; r <= rounds; ++r) {
      ...
      MultiplyWithGF2Matrix(dasta.LinMatrices_u8[r - 1], state);
      sboxlayer(state);
      print_noise(state);
    }

    std::cout << "final add" << std::endl;
    MultiplyWithGF2Matrix(dasta.LinMatrices_u8[rounds], state);
...
// void DASTA_128_HElib::sboxlayer(helib::Ctxt& state) {
//   NTL::ZZX p;
...
void DASTA_128_HElib::MultiplyWithGF2Matrix(
