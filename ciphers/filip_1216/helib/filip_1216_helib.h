...
class FiLIP_1216_HElib : public HElibCipher {
 public:
  typedef FiLIP_1216 Plain;
  FiLIP_1216_HElib(std::vector<uint8_t> secret_key,
                   std::shared_ptr<helib::Context> con, long L, long c)
      : HElibCipher(FILIP_1216_PARAMS, secret_key, con, L, c) {}

  virtual ~FiLIP_1216_HElib() = default;

  virtual std::string get_cipher_name() const { return "FiLIP-1216-HElib"; }
