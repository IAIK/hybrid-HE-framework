...
std::vector<Ctxt> AGRASTA_128_HElib::HE_decrypt(
  ...
  for (size_t i = 0; i < num_block; i++) {
    agrasta.genInstance(0, i + 1);

    Ctxt state = secret_key_encrypted[0];

    for (unsigned r = 1; r <= rounds; ++r) {
      std::cout << "round " << r << std::endl;
      MultiplyWithGF2Matrix(agrasta.LinMatrices_u8[r - 1], state);
      addConstant(state, agrasta.roundconstants_u8[r - 1]);
      sboxlayer(state);
      print_noise(state);
    }

    std::cout << "final add" << std::endl;
    MultiplyWithGF2Matrix(agrasta.LinMatrices_u8[rounds], state);
    addConstant(state, agrasta.roundconstants_u8[rounds]);
...
void AGRASTA_128_HElib::addConstant(Ctxt& state,
...
void AGRASTA_128_HElib::sboxlayer(helib::Ctxt& state) {
  NTL::ZZX p;
...
void AGRASTA_128_HElib::MultiplyWithGF2Matrix(
