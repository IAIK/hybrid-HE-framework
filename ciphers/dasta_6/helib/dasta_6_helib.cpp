#include "dasta_6_helib.h"

using namespace helib;

namespace DASTA_6 {

void DASTA_128_HElib::encrypt_key() {
...
std::vector<Ctxt> DASTA_128_HElib::HE_decrypt(std::vector<uint8_t>& ciphertexts,
                                              size_t bits) {
...
std::vector<uint8_t> DASTA_128_HElib::decrypt_result(
...
void DASTA_128_HElib::MultiplyPermutedWithGF2Matrix(
    const std::array<block, blocksize>& matrix,
    const std::array<uint16_t, blocksize>& perm, std::vector<Ctxt>& state) {
  std::vector<Ctxt> out;
...
void DASTA_128_HElib::MultiplyWithGF2Matrix(const mzd_t* matrix,
...
void DASTA_128_HElib::add(std::vector<Ctxt>& state,
...
void DASTA_128_HElib::sboxlayer(std::vector<Ctxt>& state) {
  std::vector<Ctxt> out;
